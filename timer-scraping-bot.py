...
if response.status_code == 200:
    ...
    # Sayfadaki başlığı bulma (Örneğin <h1> etiketi ile)
    page_title = soup.find('h1').text.strip()  # Başlık <h1> etiketinden alınıyor
    
    # Sayfadaki resimleri bulma
    ...
    for idx, img_url in enumerate(image_urls):
        # Rastgele bekleme süresi ekleme (1-3 saniye arası)
        wait_time = random.uniform(1, 3)
        print(f"Bekliyorum: {wait_time:.2f} saniye...")
        time.sleep(wait_time)
        
        img_response = requests.get(img_url, headers=headers)
        
        if img_response.status_code == 200:
            # Başlıkla resim dosya adını değiştirme
            img_name = os.path.join(image_folder, f"{page_title}_{idx + 1}.jpg")
